...
namespace Dapper.Contrib.Extensions
{
    ...
    public static partial class SqlMapperExtensions
    {
        ...
        public static bool UpdateChangedFields<T>(this IDbConnection connection, T entityToUpdate, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            {
                var property = keyProperties[i];
                adapter.AppendColumnNameEqualsValue(sbWhere, property.Name);  //fix for issue #336
                if (i < keyProperties.Count - 1)
                    sbWhere.Append(" and ");
            }
            ...
            try
            {
                ...
                var entity = connection.QueryFirstOrDefault<T>($"select * from {name} {sbWhere.ToString()}", entityToUpdate);
                if (entity == null)
                {
                    return false;
                }
                ...
                for (var i = 0; i < nonIdProps.Count; i++)
                {
                    ...
                    if (entityValue == null && entity2Value == null ? false : entityValue == null ? !entity2Value.Equals(entityValue) : !entityValue.Equals(entity2Value))
                    {
                        adapter.AppendColumnNameEqualsValue(sb, property.Name);  //fix for issue #336                    
                        sb.Append(",");
                        updateResult = true;
